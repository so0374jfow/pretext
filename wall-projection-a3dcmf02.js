...
// pages/demos/wall-projection.ts
var fragments = [
...
if (!wall)
  throw new Error("Missing #wall element");
var MEASURE_FONT = '17px "EB Garamond", Georgia, serif';
var LINE_HEIGHT = 34;
var MAX_VISIBLE_LINES = 7;
var state = {
  lines: [],
  currentFragment: 0,
  charIndex: 0,
...
function getWallWidth() {
  return wall.clientWidth - 72;
}
function reflowFragment(text) {
...
function renderWall() {
  ...
  displayLines.forEach((line, i) => {
    ...
    const isLastLine = i === displayLines.length - 1 && state.phase === "typing";
    if (isLastLine) {
  ...
  switch (state.phase) {
    ...
    case "pause": {
      if (now - state.pauseStart > 3500) {
        ...
        state.lines.push(...fragmentLines, "");
        state.currentFragment = (state.currentFragment + 1) % fragments.length;
        state.charIndex = 0;
        state.phase = "typing";
        state.lastTick = now;
      }
