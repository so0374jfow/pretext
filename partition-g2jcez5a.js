...
var LIGHTS = [
  { x: 200, y: -600, z: -400, intensity: 1, radius: 2000, warm: true },
...
var rng = mulberry32(3405691582);
var rand = (lo, hi) => lo + rng() * (hi - lo);
var pick = (a) => a[Math.floor(rng() * a.length)];
function generateArchitecture() {
  ...
  for (let i = 0;i < 12; i++) {
    ...
    els.push({
      ...
      rx: rand(-12, 12),
      ry: rand(-20, 20),
      rz: rand(5, 25) * (rng() > 0.5 ? 1 : -1),
      kind: "cantilever",
      text: rng() > 0.5 ? pick(FRAGMENTS) : null,
      textFace: "top",
      fontSize: rand(9, 13),
      lineHeight: rand(14, 20)
    });
  }
  return els;
}
...
function buildGroundGrid(world) {
  const size = 4000;
  const step = 200;
  const y = 100;
...
function buildLightMarkers(world) {
  for (const light of LIGHTS) {
...
var camera = { x: 0, y: -250, z: -100, rotX: 0, rotY: 0 };
var keys = {};
